...
void ArchiveHeapLoader::cleanup_regions(ArchiveHeapRegions* heap_regions) {
...
void ArchiveHeapLoader::cleanup() {
  cleanup_regions(&_closed_heap_regions);
  cleanup_regions(&_open_heap_regions);
}
...
bool ArchiveHeapLoader::is_pointer_patching_needed(FileMapInfo* map_info) {
  ...
  if (_open_heap_regions.is_mapped() && _open_heap_regions.is_relocated()) {
    log_info(cds)("CDS heap data needs to be relocated.");
    return true;
  }
...
void ArchiveHeapLoader::map_heap_regions(FileMapInfo* map_info) {
  ...
  if (!get_heap_range_for_archive_regions(&_open_heap_regions, true)) {
    // do not return as we can still map closed heap regions
  ...
  if (map_heap_regions(map_info, &_closed_heap_regions)) {
    ...
    if (_open_heap_regions.is_runtime_space_reserved() && map_heap_regions(map_info, &_open_heap_regions)) {
...
ArchiveOopDecoder* ArchiveHeapLoader::get_oop_decoder(FileMapInfo* map_info) {
  if (ArchiveHeapLoader::closed_regions_mapped() && !_oop_decoder) {
