...
void AdapterHandlerLibrary::initialize() {
  ...
  AdapterBlob* obj_obj_arg_blob = nullptr;
  {
    ...
    _no_arg_handler = create_simple_adapter(no_arg_blob, 0, nullptr, true);

    BasicType obj_args[] = { T_OBJECT };
    _obj_arg_handler = create_simple_adapter(obj_arg_blob, 1, obj_args, true);

    BasicType int_args[] = { T_INT };
    _int_arg_handler = create_simple_adapter(int_arg_blob, 1, int_args, true);

    BasicType obj_int_args[] = { T_OBJECT, T_INT };
    _obj_int_arg_handler = create_simple_adapter(obj_int_arg_blob, 2, obj_int_args, true);

    BasicType obj_obj_args[] = { T_OBJECT, T_OBJECT };
    _obj_obj_arg_handler = create_simple_adapter(obj_obj_arg_blob, 2, obj_obj_args, true);
...
void AdapterHandlerEntry::restore_unshareable_info(TRAPS) {
  ...
  // A fixed set of simple adapters are eagerly linked to the generated code during
  // JVM initialization in AdapterHandlerTable::initialize().
