...
bool AOTCodeCache::store_code_blob(CodeBlob& blob, AOTCodeEntry::Kind entry_kind, uint id, const char* name, int entry_offset_count, int* entry_offsets) {
  ...
  log_info(aot, codecache, stubs)("Writing blob '%s' (id=%u) to AOT Code Cache", name, id);
  ...
  log_info(aot, codecache, stubs)("Wrote code blob '%s' (id=%u) to AOT Code Cache", name, id);
  return true;
}
...
#define _extrs_max 50
#define _stubs_max 3

#define _shared_blobs_max 4
#define _C1_blobs_max 1
...
#ifdef COMPILER1
  // Runtime1 Blobs
  SET_ADDRESS(_C1_blobs, Runtime1::entry_for(C1StubId::forward_exception_id));
#endif // COMPILER1
...
address AOTCodeAddressTable::address_for_id(int idx) {
  ...
  if (id >= _shared_blobs_base && id < _shared_blobs_base + _shared_blobs_max) {
    return _shared_blobs_addr[id - _shared_blobs_base];
  }
  if (id >= _C1_blobs_base && id < _C1_blobs_base + _C1_blobs_max) {
