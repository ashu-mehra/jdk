...
// This version requires locking.
inline HeapWord* ContiguousSpace::allocate_aligned_impl(size_t size, size_t alignment) {
  assert(Heap_lock->owned_by_self() ||
         (SafepointSynchronize::is_at_safepoint() && Thread::current()->is_VM_thread()),
         "not locked");
...
inline HeapWord* OffsetTableContigSpace::allocate_aligned(size_t size, size_t alignment) {
  HeapWord* res = ContiguousSpace::allocate_aligned(size, alignment);
  if (res != NULL) {
